...
def user_identity_lookup(user):
  return user.id
  # user = User.Query.filterby(id=user.id).first()
  # if user:
...
#Task 8.1
# custom decorator authorize routes for admin or regular user
#Letting certain user, access certian routes
def login_required(required_class):
  ...
  def wrapper(f):
    ...
    @jwt_required()  # Ensure JWT authentication
    def decorated_function(*args, **kwargs):
      user = User.query.get(get_jwt_identity()) #pull the user name upon decryption 
      #instead of the ID is vague whereas the username is set to unique
      if user.__class__ != required_class:  # Check class equality
        return jsonify(message='Invalid user role'), 403
      return f(*args, **kwargs)
...
#Task 8.4 - Updating the action route 'Log in'
